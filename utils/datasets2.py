...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        print('yh_debug_lenimg', len(self.img_files))
        # Check cache
    ...
    def __getitem__(self, index):
        ...
        #print('yh_debugg', mosaic, self.augment)
        mosaic = False
        #self.augment = False
        if mosaic:
            # Load mosaic
        ...
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img = np.ascontiguousarray(img)
        #print('yh_debugg2', img.shape)
        return torch.from_numpy(img), labels_out, self.img_files[index], shapes

    @staticmethod
    def collate_fn(batch):
        img, label, path, shapes = zip(*batch)  # transposed
        #print('debug_len', len(img), len(label))
...
def load_image(self, index):
    ...
    img = self.imgs[index]
    #if(img is None):
    #    print('yh_debug-load None')
    #else:
    #    print('yh_debug-load', img.shape)
    if img is None:  # not cached
        ...
        min_size= 128
        #print('yh_debug_resize2', th0, tw0, temp)
        th0, tw0 =(1-(th0/min_size - th0//min_size))*min_size+th0, (1-(tw0/min_size - tw0//min_size))*min_size+tw0
        #if r != 1:  # always resize down, only resize up if training with augmentation
            #interp = cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR
            #img = cv2.resize(img, (int(w0 * r), int(h0 * r)), interpolation=interp)
        img = cv2.resize(img, (int(tw0), int(th0)), interpolation=cv2.INTER_LINEAR)
        #print('yh_debug_resize3', th0, tw0, img.shape, temp)
...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    #print('yh_debug_letterbox', new_shape.shape)
    new_shape = shape
    '''temp = random.randint(0,2)
    h0, w0 = shape
    if(temp % 3) == 0:
    ...
    th0, tw0 = shape
    th0, tw0 =(1-(th0/min_size - th0//min_size))*min_size+th0, (1-(tw0/min_size - tw0//min_size))*min_size+tw0
    new_unpad2 = int(round(shape[1] * r)), int(round(shape[0] * r))
    new_unpad = (int(tw0), int(th0))
    #print(new_unpad, new_unpad2)
